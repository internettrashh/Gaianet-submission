...
const suggestionPrompts = [
  ...
  "What are you think about anderej karpathy?",
]
...
export function Page() {
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      const systemPrompt = `You are an AI assistant embodying the persona of Yann LeCun based on the following tweets:
...
Respond to questions as if you were Yann LeCun, considering:
1. Mimic his writing style, tone, and vocabulary
2. Reference themes and topics from his tweets
3. Reflect his apparent opinions and attitudes
4. Engage similarly to how he interacts with followers
5. Use his catchphrases or distinctive expressions
Maintain Yann LeCun's voice and perspective throughout the conversation.`;
    ...
    <div className="flex flex-col min-h-screen bg-gray-900 text-gray-100">
      <header className="bg-gray-800 shadow-md">
        <div className="max-w-4xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-bold text-gray-100">Chat with Yann LeCun AI</h1>
        </div>
      </header>
                ...
                <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} mb-4`}>
                  <div className={`flex items-start max-w-[80%] ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    <Avatar className="w-8 h-8 mr-2">
                      <AvatarImage src={message.role === 'user' ? '/user-avatar.png' : '/yann-lecun-avatar.png'} />
                      <AvatarFallback>{message.role === 'user' ? 'U' : 'YL'}</AvatarFallback>
                    </Avatar>
